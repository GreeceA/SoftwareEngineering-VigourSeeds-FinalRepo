...
export default function Edit({ auth, contract, partners, seeds }) {
    const fileInputRef = useRef();
    const contractSeedItems = contract.contractSeedItems ?? [];

    // Partner search state
    const [partnerSearch, setPartnerSearch] = useState(
        partners.find(p => p.id === contract.partner_id)?.name || ''
    );
    ...
    // Selected seeds state (full objects for UI)
    const [selectedSeeds, setSelectedSeeds] = useState(
        contractSeedItems.map(item => {
            const baseSeed = seeds.find(s => s.id === item.seed_id) || item.seed || {};
            return {
                ...baseSeed,
                id: item.seed_id,
                ...
                growth_cycle: Number(item.seed?.growth_cycle ?? baseSeed.growth_cycle ?? 0),
                price_per_unit: item.seed?.price_per_unit ?? baseSeed.price_per_unit ?? 0,
            };
        })
    );
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        contract_file: null,
        seeds: contractSeedItems.map(item => ({
    ...
    // Keep form seeds in sync with selectedSeeds
    useEffect(() => {
        setData('seeds', selectedSeeds.map(seed => ({
            seed_id: seed.id,
    ...
    // Always set partner_id on mount
    useEffect(() => {
        setData('partner_id', contract.partner_id || '');
        setPartnerSearch(partners.find(p => p.id === contract.partner_id)?.name || '');
    }, [contract.partner_id, partners]);

    // Partner search/filter logic
    ...
    const handleSeedSelection = (seed) => {
        ...
        setSelectedSeeds([...selectedSeeds, newSeed]);
    };
    ...
    // Update a field in a selected seed
    const updateSeedData = (seedId, field, value) => {
        setSelectedSeeds(selectedSeeds.map(seed => {
            if (seed.id === seedId) {
        ...
        >
            ...
            <div className="px-6 pt-6">
                <nav className="text-sm text-gray-600">
                    ...
                    / <Link href={route('contracts.index')} className="text-[#37692F] hover:underline">Contracts</Link> / <span>Edit Contract</span>
                </nav>
            </div>
                            ...
                            <div>
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    {contract.contract_file && (
                                        <a
                                            ...
                                            download={contract.original_file_name || true}
                                            className="ml-2 bg-[#37692F] hover:bg-[#2a5624] text-white px-2 py-2 rounded transition-colors flex items-center"
                                ...
                                {contract.original_file_name && (
                                    <div className="text-xs text-gray-700 mt-1">
                                        <span className="font-semibold">Current file:</span> {contract.original_file_name}
                                    </div>
                                )}
